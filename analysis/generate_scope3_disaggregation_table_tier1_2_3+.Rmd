...
This block handles all setup, including package installation and dynamically fetching model specifications from GitHub based on the user's selected `sef_version`. For v1.3.0 and older the specs come from `USEPA/supply-chain-factors`; for v1.4.0 and newer from `cornerstone-data/supply-chain-factors`.
...
# --- 3. Fetch Versioning.yml (Cornerstone first, USEPA as fallback) ---
# Cornerstone is the primary home of SEF v1.4.0+. Their Versioning.yml also
# contains all older versions (v1.1.1–v1.3.0), so it serves as a single source
# of truth. USEPA's copy is kept as a fallback for older versions.
versioning_urls <- c(
  "https://raw.githubusercontent.com/cornerstone-data/supply-chain-factors/main/Versioning.yml",
  "https://raw.githubusercontent.com/USEPA/supply-chain-factors/main/Versioning.yml"
)
versioning_data <- NULL
for (versioning_url in versioning_urls) {
  response <- tryCatch(GET(versioning_url), error = function(e) NULL)
  if (!is.null(response) && !http_error(response)) {
    versioning_data <- read_yaml(text = content(response, "text", encoding = "UTF-8"))
    cli::cli_alert_info("Loaded Versioning.yml from: {versioning_url}")
    break
  }
}
if (is.null(versioning_data)) {
  stop("Could not access Versioning.yml from any known URL. Please check your internet connection.")
}
...
# --- 4. Resolve GitHub org and install correct `useeior` version ---
# useeior_org defaults to "cornerstone-data" (the permanent home from v1.4.0 onward).
# For older SEF versions hosted by USEPA, set useeior_org: "USEPA" in model_spec_mapping.yml.
model_map_early <- tryCatch(read_yaml("model_spec_mapping.yml"), error = function(e) NULL)
ver_config  <- if (!is.null(model_map_early)) model_map_early$sef_version_config[[config$sef_version]] else NULL
useeior_org <- if (!is.null(ver_config) && !is.null(ver_config$useeior_org)) ver_config$useeior_org else "cornerstone-data"

useeior_tag <- versioning_data[[config$sef_version]][["useeior_tag"]]
useeior_ver <- versioning_data[[config$sef_version]][["useeior_ver"]]
install_target <- paste0(useeior_org, "/useeior@", useeior_tag)

if (!"devtools" %in% installed.packages()[, "Package"]) {
  suppressMessages(install.packages("devtools"))
}

# Compare by RemoteRef + RemoteUsername (stored in DESCRIPTION by devtools::install_github).
# This works correctly for both semver tags ("v1.5.3") and named tags ("SEFv1.4"), and
# also catches org switches (USEPA → cornerstone-data), avoiding unnecessary reinstalls.
needs_install <- TRUE
if ("useeior" %in% installed.packages()[, "Package"]) {
  desc_path <- system.file("DESCRIPTION", package = "useeior")
  if (nzchar(desc_path)) {
    desc_fields <- read.dcf(desc_path, fields = c("RemoteRef", "RemoteUsername", "Version"))
    installed_ref <- as.character(desc_fields[, "RemoteRef"])
    installed_org <- as.character(desc_fields[, "RemoteUsername"])
    if (!is.na(installed_ref) && identical(installed_ref, useeior_tag) &&
        !is.na(installed_org) && identical(installed_org, useeior_org)) {
      needs_install <- FALSE
      cli::cli_alert_success(
        "useeior {useeior_ver} (tag @{useeior_tag}, org {useeior_org}) is already installed."
      )
    } else {
      cli::cli_alert_warning(
        "Installed useeior is from {installed_org}@{installed_ref}; need {useeior_org}@{useeior_tag}. Reinstalling..."
      )
    }
  }
}
if (needs_install) {
  cli::cli_alert_info("Installing useeior {useeior_ver} (tag @{useeior_tag}) from {useeior_org}...")
  suppressMessages(devtools::install_github(install_target, quiet = TRUE))
}
...
if (!is.null(model_map) && !is.null(model_map$model_name_by_sef_version[[config$sef_version]])) {
  ...
  msg <- paste0(
    ...
    "For example:\n",
    "  \"", config$sef_version, "\": \"USEEIOvX.Y.Z-name\"\n\n",
    "How to find the correct USEEIO model name: \n",
    " - Check the SEF release notes and/or Versioning.yml in cornerstone-data/supply-chain-factors.\n",
    " - URL: https://raw.githubusercontent.com/cornerstone-data/supply-chain-factors/main/Versioning.yml\n",
    " - Identify the model spec name used for that SEF release (e.g., USEEIOv2.2.22-GHG or USEEIOv2.6.0-phoebe-23).\n"
  )
  stop(msg)
}
...
if (!file.exists(local_spec_file)) {
  ...
  # Route to the correct org's supply-chain-factors repo (set in chunk 01)
  sef_repo_org <- if (exists("useeior_org")) useeior_org else "cornerstone-data"
  model_spec_url <- paste0(
    "https://raw.githubusercontent.com/", sef_repo_org,
    "/supply-chain-factors/main/model-specs/",
    model_name, ".yml"
  )
...
# --- 3. Build the model ---
# configpaths must include both the local model spec AND the package's disaggspecs
# directory, which is needed by newer models (e.g. USEEIOv2.6.0-phoebe-23) that
# reference bundled disaggregation specs like WasteDisaggregationDetail2017.yml.
disagg_spec_dir <- system.file("extdata/disaggspecs", package = "useeior")
config_paths <- c(local_spec_file)
if (nzchar(disagg_spec_dir) && dir.exists(disagg_spec_dir)) {
  config_paths <- c(config_paths, disagg_spec_dir)
}
cli::cli_alert_info("Building USEEIO model: {model_name} (this can take a minute)...")
model <- buildModel(model_name, configpaths = config_paths)
...
csv_sc_filename   <- file.path(csv_dir, paste0("SEF_",  config$sef_version,"_sector_classification.csv"))
...
if (useeior_org == "USEPA") {
  ack_note        <- "This project builds on open-source work by the U.S. EPA (see below)"
  ack_useeior_url <- "https://github.com/USEPA/useeior (MIT License, Copyright U.S. EPA)"
  ack_sef_url     <- "https://github.com/USEPA/supply-chain-factors (MIT License, Copyright U.S. EPA)"
} else {
  ack_note        <- "This project builds on open-source work by the U.S. EPA (USEEIO framework) and Cornerstone (SEF v1.4.0+, see below)"
  ack_useeior_url <- "https://github.com/cornerstone-data/useeior (MIT License, Cornerstone; original: USEPA/useeior)"
  ack_sef_url     <- "https://github.com/cornerstone-data/supply-chain-factors (MIT License, Cornerstone; original: USEPA/supply-chain-factors)"
}
