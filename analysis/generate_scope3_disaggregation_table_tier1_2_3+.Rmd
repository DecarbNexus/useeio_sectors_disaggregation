...
json_dir <- file.path(outputs_dir, "json")
jsonld_dir <- file.path(outputs_dir, "jsonld")
for (d in c(csv_dir, parquet_dir, json_dir, jsonld_dir)) {
...
log_step("Writing structured outputs (Parquet, JSON, JSON-LD)...")
...
# 6.2 Hierarchical JSON: nested structure preserving commodity > tier > sector > scope
# Build nested hierarchy for cleaner structure
json_start <- Sys.time()
log_step("Building hierarchical JSON (nesting commodity > tier > sector > scope)...")

nested_json <- final_table_by_code %>%
  rename(absolute_contribution = Absolute_Contribution_GHG) %>%
  group_by(Disaggregated_Commodity_Code) %>%
  summarise(
    tiers = list(
      cur_data() %>%
        group_by(Economic_Tier) %>%
        summarise(
          sectors = list(
            cur_data() %>%
              group_by(Embedded_Sector_Code, Embedded_Sector_Code_for_GHG_sources) %>%
              summarise(
                scopes = list(
                  cur_data() %>%
                    transmute(
        ...
        rename(tier = Economic_Tier)
    ),
    .groups = "drop"
  ) %>%
  rename(commodity_code = Disaggregated_Commodity_Code)

# Sector classification as flat JSON (no further nesting needed)
sc_json <- sector_classification %>%
  rename_with(~gsub(" ", "_", tolower(.)), everything())

log_step(sprintf("Hierarchy built (%s commodities); writing JSON files...", nrow(nested_json)))

json_filename <- file.path(json_dir, paste0(file_basename, ".json"))
json_sc_filename <- file.path(json_dir, "sector_classification.json")
jsonlite::write_json(
  nested_json,
  json_filename,
  pretty = TRUE,
...
json_elapsed <- round(as.numeric(difftime(Sys.time(), json_start, units = "secs")), 2)
log_step(sprintf("Wrote JSON (2 files) in %.2f seconds", json_elapsed))

# 6.3 JSON-LD: RDF-ready with @context for knowledge graph ingestion
jsonld_start <- Sys.time()
log_step("Building JSON-LD with @context vocabularies...")

# Define vocabulary for USEEIO disaggregation factors
jsonld_context <- list(
  ...
  "ghg" = "http://www.wikidata.org/entity/",
...
# Sector classification JSON-LD with simple schema.org vocabulary
...
log_step(sprintf("Wrote JSON-LD (2 files) in %.2f seconds", jsonld_elapsed))

total_structured_elapsed <- parquet_elapsed + json_elapsed + jsonld_elapsed
log_step(sprintf("All structured outputs complete in %.2f seconds total", total_structured_elapsed))

cat("Saved structured outputs:\n")
cat(" - Parquet (data):  ", parquet_filename, "\n")
cat(" - Parquet (class): ", parquet_sc_filename, "\n")
cat(" - JSON (data):     ", json_filename, "\n")
cat(" - JSON (class):    ", json_sc_filename, "\n")
cat(" - JSON-LD (data):  ", jsonld_filename, "\n")
cat(" - JSON-LD (class): ", jsonld_sc_filename, "\n")
```
